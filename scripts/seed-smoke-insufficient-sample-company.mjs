...
const IDS = {
  company: '99999999-9999-4999-8999-999999999101',
  campaign: '99999999-9999-4999-8999-999999999301',
  publicDiagnostic: '99999999-9999-4999-8999-999999999401',
  paymentOrder: '99999999-9999-4999-8999-999999999501',
};

const TOKENS = {
  companyForm: 'smoke_amostra_insuficiente_empresa',
  employeeForm: 'smoke_amostra_insuficiente_colaborador',
  publicDiagnostic: 'smoke_amostra_insuficiente_pdf_pago',
};

const responses = [
  { id: '99999999-9999-4999-8999-999999999601', scores: { 'Sobrecarga e ritmo': 20, 'Assédio e violência': 35 } },
  { id: '99999999-9999-4999-8999-999999999602', scores: { 'Sobrecarga e ritmo': 30, 'Assédio e violência': 40 } },
  { id: '99999999-9999-4999-8999-999999999603', scores: { 'Sobrecarga e ritmo': 25, 'Assédio e violência': 32 } },
];
  ...
  try {
    ...
    await client.query(
      `insert into companies
       ...
       values
        ($1, 'SMOKE Empresa Maior Amostra Insuficiente Ltda', 'SMOKE Amostra Insuficiente', '00.000.000/0009-49', 'Goiania', 'GO', 'Smoke test de empresa maior com coleta insuficiente', '20', 'Validador Amostra', 'smoke.amostra@example.com', null, 'ativa')
    ...
    await client.query(
      `insert into campaigns
       ...
       values
        ($1,$2,'SMOKE Empresa Maior - Amostra insuficiente','Campanha local para validar bloqueio em empresa com quadro maior, mas poucas respostas.',now(),now() + interval '90 days','completo','ativa',null,null,$3,$4)
       on conflict (id) do update set
    ...
    for (const response of responses) {
      await client.query(
        `insert into employee_responses
         ...
         values
          ($1,$2,$3,'Grupo parcial','Nao informado','Nao informado','Nao informado','Nao informado',null,1,'{}',$4)`,
    ...
    await client.query(
      `insert into public_diagnostics
       ...
       values
        ($1,$2,$3,$4,'Validador Amostra','smoke.amostra@example.com',null,'paid')
       on conflict (id) do update set
        company_id = excluded.company_id,
    ...
    await client.query(
      ...
      [
        ...
        IDS.campaign,
        reportPriceCents,
        'smoke-insufficient-sample-preference-approved',
        `${APP_URL}/diagnostico/${TOKENS.publicDiagnostic}/pagamento?status=approved`,
        IDS.paymentOrder,
        'smoke-insufficient-sample-approved-payment',
        'smoke.amostra@example.com',
        JSON.stringify({ smoke: true, mode: 'insufficient-sample-large-company', status: 'approved' }),
      ]
    );
