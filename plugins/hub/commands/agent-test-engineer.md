---
description: Dispatch hub:test-engineer — Expert in testing, TDD, and test automation.
argument-hint: <task or prompt for the agent>
---

# /hub:agent-test-engineer

Direct dispatch shim for the `hub:test-engineer` agent.

## Flow

1. If `$ARGUMENTS` is empty, ask the user: *"What would you like hub:test-engineer to do?"* Wait for a reply, then continue with the reply as the prompt.
2. Dispatch the agent with the user's input:

   ```
   Agent(subagent_type="hub:test-engineer", prompt="$ARGUMENTS")
   ```
