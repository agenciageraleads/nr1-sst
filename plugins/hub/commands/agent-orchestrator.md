---
description: Dispatch hub:orchestrator — Multi-agent coordination and task orchestration.
argument-hint: <task or prompt for the agent>
---

# /hub:agent-orchestrator

Direct dispatch shim for the `hub:orchestrator` agent.

## Flow

1. If `$ARGUMENTS` is empty, ask the user: *"What would you like hub:orchestrator to do?"* Wait for a reply, then continue with the reply as the prompt.
2. Dispatch the agent with the user's input:

   ```
   Agent(subagent_type="hub:orchestrator", prompt="$ARGUMENTS")
   ```
