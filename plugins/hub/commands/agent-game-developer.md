---
description: Dispatch hub:game-developer — Game development across all platforms (PC, Web, Mobile, VR/AR).
argument-hint: <task or prompt for the agent>
---

# /hub:agent-game-developer

Direct dispatch shim for the `hub:game-developer` agent.

## Flow

1. If `$ARGUMENTS` is empty, ask the user: *"What would you like hub:game-developer to do?"* Wait for a reply, then continue with the reply as the prompt.
2. Dispatch the agent with the user's input:

   ```
   Agent(subagent_type="hub:game-developer", prompt="$ARGUMENTS")
   ```
