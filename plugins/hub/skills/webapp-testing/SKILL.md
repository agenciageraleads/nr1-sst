...
| Stack | In-Process Client | E2E Driver |
|-------|-------------------|------------|
| **Node (Express/Fastify)** | `supertest` | Playwright |
| **Next.js** | Route handler unit tests | Playwright |
| **FastAPI** | `httpx.AsyncClient(app=app)` or `TestClient` | Playwright against `uvicorn` |
| **Django** | `django.test.Client` / DRF `APIClient` | Playwright against `runserver` |
...
| Pattern | Example (Node) | Example (Python) |
|---------|----------------|------------------|
| Feature-based | `login.spec.ts` | `test_login.py` |
| Descriptive | `user-can-checkout.spec.ts` | `test_user_can_checkout.py` |
