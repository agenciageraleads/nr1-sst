...
| Stack | Unit/Integration | Mocking | HTTP Client |
|-------|------------------|---------|-------------|
| **Node/TS** | Vitest / Jest | `vi.mock` / `jest.mock` | supertest |
| **Python** | pytest (+ `pytest-asyncio`) | `unittest.mock` / `pytest-mock` | httpx `AsyncClient` |
| **Rust** | `cargo test` | `mockall` | `reqwest` |
| **Go** | `go test` | `gomock` / `testify/mock` | `httptest` |

> **FastAPI tip:** Use `asyncio_mode = "auto"` in `pyproject.toml` so you don't need `@pytest.mark.asyncio` on every test.
